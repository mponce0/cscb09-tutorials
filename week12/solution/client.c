...
int main(int argc, char* argv[]) {
	...
	while(1) {
		...
		if (FD_ISSET(sockfd, &rset)) {
			int len = read(sockfd, &server_counter, sizeof(server_counter));
			printf("%d\n", server_counter);
			if (len <= 0) {
				perror("Read Error from server, closing connection");
				exit(1);
			}
		}

		if (FD_ISSET(fileno(stdin), &rset)) {
			if (read(fileno(stdin), buf, BUF_SIZE) <= 0) {
			...
			if (write(sockfd, &server_counter, sizeof(server_counter)) < 0) {
				perror("write");
				exit(1);
			}
		}
